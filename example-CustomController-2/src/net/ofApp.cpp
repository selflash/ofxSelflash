...
void ofApp::setup()
{
	...
	//ネットワーク
	//バッチ処理用
	networkManager->connectWithUDP(0, "127.0.0.1", 22222);
	...
	//{
	//	for (int i = 0; i < NUM_PARTICLES; i++) {
	//		calibPoints4[i] = vec3(0, 0, 0);
	//		calibPointsColor4[i].set(0, 0, 1, 0);
	//	}
	//	vbo4.setVertexData(calibPoints4, NUM_PARTICLES, GL_DYNAMIC_DRAW);
	//	vbo4.setColorData(calibPointsColor4, NUM_PARTICLES, GL_DYNAMIC_DRAW);
	//}
	...
	if (numKinect >= 4) {
		//kinect4.camera.setPosition(0.f, 0.f, 2000.f);
		kinect4.setup();
		kinect4.camera = &camera;
		if (kinect4.open(2)) {
			if (kinect4.isMasterDevice()) {
				masterKinect = &kinect4;
			}
			else {
				if (kinect4.startCameras(K4A_WIRED_SYNC_MODE_SUBORDINATE)) {
					cout << "this kinect(" + kinect4.device.getSerialNum() + ") has started." << endl;
				}
			}
		}
	}
...

	{
		...
		if (
			...
			(
				...
				calibrationMode == Device4ToDevice1 ||
			...
			)
		{
			...
			const ofPixels& colorInDepthPix = kinect4.device.getColorInDepthPix();
			const size_t width = colorInDepthPix.getWidth();
			const size_t height = colorInDepthPix.getHeight();
			if (markerDetector4.findMarker(colorInDepthPix, width, height, tempPoints2d)) {
				int i = 0;
				for (const auto& p : tempPoints2d) {
					if (kinect4.device.depthToWorld(p.x, p.y, tempVec3)) {
						tempPoints3d[i] = tempVec3;
					}
					i++;
				}
		...
		if (calibrationMode == Ground) {
			...
			if (
				...
				if (detectedMarker1 && detectedMarker4) {
					for (auto p : markerPoints1) {
						poseEstimater.corners14_1.push_back(p);
					}
					for (auto p : markerPoints4) {
						poseEstimater.corners14_4.push_back(p);
					}
					...
					if (poseEstimater.corners14_1.size() >= NUM_PARTICLES) {
						endCalibration();
					}
				}
			}
		...
		if (ofGetKeyPressed('h')) {
			const vector<vec3>& vertices = kinect1.device.getPositionCache();
			...
			//if (numVertices > 100) {
				...
				cout << "data = " << data << endl;
				networkManager->sendMessageWithUDP(0, data);
			//}
		}
...
void ofApp::drawInfo() {
	...
	else if (calibrationMode == Device4ToDevice1)
	{
		ofDrawBitmapStringHighlight("Is Calibrating = " + ofToString(isCalibrating ? "True" : "False"), 10, iy + 20 * 0);
		ofDrawBitmapStringHighlight("Detected Maker On Kinect1 = " + ofToString(detectedMarker1 ? "True" : "False"), 10, iy + 20 * 1);
		ofDrawBitmapStringHighlight("Detected Maker On Kinect3 = " + ofToString(detectedMarker4 ? "True" : "False"), 10, iy + 20 * 2);
		const auto& corners1 = poseEstimater.corners14_1;
		ofDrawBitmapStringHighlight("corners14_1.size = " + ofToString(corners1.size()), 10, iy + 20 * 3);
		const auto& corners4 = poseEstimater.corners14_4;
		ofDrawBitmapStringHighlight("corners14_4.size = " + ofToString(corners4.size()), 10, iy + 20 * 4);
	}
...
void ofApp::drawPointCloud() {
	...
	//else
	{
		...
		if (numKinect >= 4) {
			compositeShader.setUniformTexture("colorTex4", kinect4.fbo.getTexture(), 7);
			compositeShader.setUniformTexture("depthTex4", kinect4.fbo.getDepthTexture(), 8);
		}
		fbo.draw(0, 0);
		compositeShader.end();
...
void ofApp::drawMarkers() {
	...
	{
		drawMarker(kinect4, markerPoints4, ofColor(255, 0, 0, 255));
	}
}
...
void ofApp::drawCalibrationPoints() {
	...
	{
		kinect4.camera->begin();
		ofDisableDepthTest();

		glPointSize(2.0);
		vbo4.draw(GL_POINTS, 0, NUM_PARTICLES);
...
void ofApp::startCalibration() {
	...
	if (calibrationMode == Device2ToDevice1) {
		...
		if (numKinect >= 4) {
			mat4 mat;
			kinect1.setTransformMat(mat);
			kinect4.setTransformMat(mat);
			//poseEstimater.saveMatrix("matrixes/transMat1.txt", mat);

			poseEstimater.corners14_1.clear();
			poseEstimater.corners14_4.clear();
			...
			//for (int i = 0; i < NUM_PARTICLES; i++) {
			//	calibPoints4[i] = vec3(0, 0, 0);
			//	calibPointsColor4[i].a;
			//}
			//vbo3.updateVertexData(calibPoints4, NUM_PARTICLES);
			//vbo3.updateColorData(calibPointsColor4, NUM_PARTICLES);
...
void ofApp::endCalibration() {
	...
	if (calibrationMode == Device2ToDevice1) {
		if (numKinect >= 2) {
			mat4x4 mat = poseEstimater.estimate2to1();
			kinect2.setTransformMat(mat);
			poseEstimater.saveMatrix("matrixes/transMat2.txt", mat);
		...
		if (numKinect >= 4) {
			mat4x4 mat = poseEstimater.estimate4to1();
			kinect4.setTransformMat(mat);
			poseEstimater.saveMatrix("matrixes/transMat4.txt", mat);

			//for (int i = 0; i < NUM_PARTICLES; i++) {
			//	vec3 p = calibPoints4[i];
			//	calibPoints4[i] = vec3(mat * vec4(p, 1));
			//}
			//vbo4.updateVertexData(calibPoints4, NUM_PARTICLES);
		}
	}
	else if (calibrationMode == Ground) {
		if (detectedMarker1) {
			...
			{
				ofMatrix4x4 currentMat = kinect4.getTransformMat();
				currentMat = currentMat * ofMatrix4x4(mat_1toWorld);
				mat4x4 mat_4toWorld = currentMat;
				kinect4.setTransformMat(mat_4toWorld);
				poseEstimater.saveMatrix("matrixes/transMat4.txt", mat_4toWorld);
			}
		}
...
void ofApp::keyPressed(int key) {
	...
	if (key == OF_KEY_UP)
	{
		...
		if (numKinect >= 4) kinect4.pointSize += 1;
	}
	...
	if (key == 'l')
	{
		...
		if (numKinect >= 4) {
			mat4x4 mat_4toWorld;
			poseEstimater.loadMatrix("matrixes/transMat4.txt", mat_4toWorld);
			kinect4.setTransformMat(mat_4toWorld);
		}
	}
