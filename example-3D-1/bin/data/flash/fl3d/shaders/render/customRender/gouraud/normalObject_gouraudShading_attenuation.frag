...
vec4 shading(int id, vec3 rowDirection, float intensity, bool enabled){
    ...
    //--------------------------------------
    //環境反射成分(全体の色。暗いところ)
    vec4 ambient = gl_Color * gl_FrontMaterial.emission * gl_LightSource[id].ambient;
...
void main(void) {
    ...
    //if(u_Light7Enabled == 1) compositeColor += shading(u_Light7ID, v_Light7Dir, u_Light7Intensity, u_Light7AttenuationEnabled);
    //--------------------------------------
